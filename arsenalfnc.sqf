/* EDITED TO WORK ON ANY UNIT/AI THAT ACTIVATES
...
switch _mode do {
	...
	case "draw3D": {
		...
		_center = (missionnamespace getvariable ["BIS_fnc_arsenal_center",CSAR_AmmoBoxUnit]);
		_target = (missionnamespace getvariable ["BIS_fnc_arsenal_target",CSAR_AmmoBoxUnit]);
		...
		if ((getposasl _cam select 2) < (getposasl _center select 2)) then {
			...
			_cam setpos (_target modeltoworldvisual [5,-_dis * _disCoef,0]);
		...
		if (BIS_fnc_arsenal_type == 0) then {
			...
			//_cam = (uinamespace getvariable ["BIS_fnc_arsenal_cam",CSAR_AmmoBoxUnit]);
			//_target = (uinamespace getvariable ["BIS_fnc_arsenal_target",CSAR_AmmoBoxUnit]);
	...
	case "Open": {
		...
		with missionnamespace do {
			...
			BIS_fnc_arsenal_center = [_this,2,CSAR_AmmoBoxUnit,[CSAR_AmmoBoxUnit]] call bis_fnc_param;
		};
		with uinamespace do {
	...
	case "Init": {
		...
		if !(is3DEN) then {
			if (_fullVersion) then {
				if (vehicle CSAR_AmmoBoxUnit != CSAR_AmmoBoxUnit) then {
					moveout CSAR_AmmoBoxUnit;
					CSAR_AmmoBoxUnitCSAR_AmmoBoxUnit switchaction "CSAR_AmmoBoxUnitstand";
					[CSAR_AmmoBoxUnit,0] call bis_fnc_setheight;
				};
				...
				if (time < 1) then {
					...
					if (isclass (configfile >> "cfgvehicles" >> _defaultClass)) then {
						...
						[CSAR_AmmoBoxUnit,_defaultClass] call bis_fnc_loadinventory;
						...
						[CSAR_AmmoBoxUnit,_soldiers call bis_fnc_selectrandom] call bis_fnc_loadinventory;
					};
	...
	case "InitGUI": {
		...
		CSAR_AmmoBoxUnit switchcamera "internal";
		...
		if (is3DEN) then {
			...
			_center switchaction "CSAR_AmmoBoxUnitstand";
		...
		_ctrlButtonExport ctrlenable !ismultiCSAR_AmmoBoxUnit;
	...
	case "Exit": {
		...
		CSAR_AmmoBoxUnit switchcamera BIS_fnc_arsenal_cameraview;
	...
	case "ListAdd": {
		...
		_listDefaults = [
			...
			[], // IDC_RSCDISPLAYARSENAL_TAB_NVGS
			[binocular CSAR_AmmoBoxUnit], // IDC_RSCDISPLAYARSENAL_TAB_BINOCULARS
			[], // IDC_RSCDISPLAYARSENAL_TAB_MAP
			[], // IDC_RSCDISPLAYARSENAL_TAB_GPS
			[], // IDC_RSCDISPLAYARSENAL_TAB_RADIO
			[], // IDC_RSCDISPLAYARSENAL_TAB_COMPASS
...
*/
	...
	case "buttonSpace": {
		...
		//missionnamespace setvariable ["BIS_fnc_arsenal_target",CSAR_AmmoBoxUnit];
		_function spawn {
