...
_smoke setVelocity [sin (_dir)*(6+random 6),cos (_dir)*(6+random 6),6+(random 5)];
...
IF (_Engageloop) then {
	...
	IF ((leader _Group) distance _Engageunit > (_BreakOffDistance + _AttackDistance) && _engageloop) then {deletewaypoint [_group,_WPIndex];_group setcurrentwaypoint [_group,(_group getvariable "CSAR_OldWP")];_Engageloop = false;CSAR_EngagedGroups = CSAR_EngagedGroups - [[_group,_pos]];_group setvariable ["CSAR_ENGAGED",false]};
