...
if (_casNum > 0) then {
	...
	PlayerCASActionID = _object addAction [_str, "Support\casMenu.sqf", [_CASMaxDist, _casNum, _basePos], -1, false, true, "","player == _target && (vehicle _target) isKindOf ""MAN"" && AATaskComplete"];
};
...
_SPTMaxDist = 1000;
PlayerSPTActionID = _object addAction [_str, "Support\sptMenu.sqf", [_SPTMaxDist, _basePos], -1, false, true, "","player == _target && (vehicle _target) isKindOf ""MAN"" && AATaskComplete"];