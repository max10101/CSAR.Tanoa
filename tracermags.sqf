if(local _this) then {
	...
	{IF ((_x select 0) in magazines _unit) then {_unit removemagazines (_x select 0);_unit addmagazines [(_x select 1),8];_unit removeprimaryweaponitem (_x select 0);_unit addprimaryweaponitem (_x select 1)}} foreach _mags;
};